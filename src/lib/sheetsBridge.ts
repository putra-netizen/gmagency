  ...
  if (webhookUrl && typeof webhookUrl === 'string' && webhookUrl.startsWith('http')) {
    try {
      ...
      if (res.ok) {
        ...
        if (json && !Array.isArray(json) && json.ok === false && json.error) {
          throw new Error(String(json.error));
        }

        // Spreadsheet is the source of truth. A successful empty sheet MUST
        // return [] rather than falling back to stale Supabase/local data.
...
export async function updateOrderFields(sheetName: string, rowId: string, fields: Row): Promise<any> {
  ...
  if (webhookUrl && typeof webhookUrl === 'string' && webhookUrl.startsWith('http')) {
    ...
    if (!res.ok || (payload && payload.ok === false)) {
      throw new Error(payload?.error || `Apps Script update failed: HTTP ${res.status}`);
    }
  }
